...
type UserRequest_StorageServices interface {
	Save(q UserRequest) (err protocol.Error)

	Count(requestID [16]byte) (numbers uint64, err protocol.Error)
	Get(requestID [16]byte, versionOffset uint64) (ur UserRequest, err protocol.Error)
	Last(requestID [16]byte) (ur UserRequest, numbers uint64, err protocol.Error)

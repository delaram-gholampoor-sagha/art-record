...
type InvoiceSummary_StorageServices interface {
	Save(is InvoiceSummary) (err error)

	Count(invoiceID [16]byte) (numbers uint64, err error)
	Get(invoiceID [16]byte, versionOffset uint64) (is InvoiceSummary, err error)
	Last(invoiceID [16]byte) (is InvoiceSummary, numbers uint64, err error)
}

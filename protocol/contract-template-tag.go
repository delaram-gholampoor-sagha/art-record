...
type ContractTemplateTag_StorageServices interface {
	Save(ct ContractTemplateTag) protocol.Error

	Count(contractTemplateID [16]byte) (numbers uint64, err protocol.Error)
	Get(contractTemplateID [16]byte, versionOffset uint64) (ct ContractTemplateTag, err protocol.Error)
	Last(contractTemplateID [16]byte) (ct ContractTemplateTag, numbers uint64, err protocol.Error)
